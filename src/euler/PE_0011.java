...
public class PE_0011 {
    ...
    private static int[][] parseNumbers() {
        String[] rows = PE_0011.INPUT.split("\n");
        int[][] parsedNumbers = new int[rows.length][];
        for (int i = 0; i < parsedNumbers.length; i++) {
            String[] rowOfNumbers = rows[i].split(" ");
            int[] rowOfNumbersToInt = new int[rowOfNumbers.length];
            for (int j = 0; j < rowOfNumbers.length; j++) {
                rowOfNumbersToInt[j] = Integer.parseInt(rowOfNumbers[j]);
            }
            parsedNumbers[i] = rowOfNumbersToInt;
        }
    ...
    private static long biggestProductOfNNumbersInAnyDirection(int n, int[][] numbers) {
        ...
        for (int i = 0; i < numbers.length-n; i++) {
            for (int j = 0; j < numbers[i].length; j++) {
                String[] directions = {"R", "D", "DR", "DL"};
                for (String direction : directions) {
                    long product = productOfNConsecutiveNumbers(numbers, n, i, j, direction);
    ...
    private static long productOfNConsecutiveNumbers(int[][] numbers, int n, int startRow, int startCol, String direction) {
        ...
        int increaseDown = 1;
        switch (direction) {
            case "R" -> increaseDown = 0;
            case "D" -> increaseRight = 0;
            case "DL" -> increaseRight = -1;
        }
        for (int i = 1; i < n; i++) {
            int rowIndex = startRow + increaseDown*i;
            int columIndex = startCol + increaseRight*i;
            if (columIndex < 0 || columIndex >= numbers[rowIndex].length) return 0;
            product *= numbers[rowIndex][columIndex];
        }
        return product;
    }
}
