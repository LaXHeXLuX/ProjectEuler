...
public class PE_0105 {
    static void main() {
        System.out.println(PE());
    }

    public static String PE() {
        String filename = "src/euler/inputs/PE_0105_sets.txt";
    ...
    public static boolean noDisjointSubSetsWithSameSum(List<Integer> set) {
        ...
        for (int i = 2; i < set.size(); i++) {
            clone.add(set.get(i));
            if (!PE_0103.noDisjointSubSetsWithSameSum(clone)) {
                return false;
            }
        }
        return true;
    ...
    private static boolean isSpecialSumSet(List<Integer> set) {
        return PE_0103.isSumOrdered(set) && noDisjointSubSetsWithSameSum(set);
    }
}
