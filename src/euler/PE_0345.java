...
public class PE_0345 {
    ...
    private static int maxMatrixSum(int[][] matrix) {
        int n = matrix.length;
        int[] bestSum = new int[1 << n];

        for (int mask = 0; mask < 1 << n; mask++) {
            int next = Integer.bitCount(mask);

            for (int i = 0; i < n; i++) {
                if ((mask & (1 << i)) != 0) continue;
                int newMask = mask | (1 << i);
                int sum = bestSum[mask] + matrix[i][next];
                if (bestSum[newMask] < sum) bestSum[newMask] = sum;
            }
        }

        return bestSum[(1 << n) - 1];
    }
